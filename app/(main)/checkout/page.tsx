...
export default function CheckoutPage() {
    ...
    useEffect(() => {
        ...
        fetch("/api/loyalty", { cache: "no-store" })
            .then((r) => r.json())
            .then(({ membership }) =>
                setLoyaltyPoints(membership?.points ?? 0),
            );
    }, [user]);
    ...
    const maxRedeemablePoints = Math.min(
        loyaltyPoints,
        Math.floor((total + shippingFee) / REDEMPTION_VALUE) * REDEMPTION_RATE,
    );
    const actualPointsToRedeem =
        useLoyalty ?
            Math.min(pointsToRedeem || maxRedeemablePoints, maxRedeemablePoints)
        :   0;
    const loyaltyDiscount =
        Math.floor(actualPointsToRedeem / REDEMPTION_RATE) * REDEMPTION_VALUE;
        ...
        return (
            <div className='min-h-screen pt-28 flex items-center justify-center'>
                <div className='text-center'>
                    ...
                    <h1 className={`text-xl font-bold ${tc} mb-2`}>
                        Your cart is empty
                    </h1>
                    <Link
                        href='/products'
    ...
    return (
        <div className='min-h-screen pt-28 pb-16'>
            <div className='max-w-5xl mx-auto px-4 sm:px-6 py-10'>
                <Link
                    href='/cart'
                    className={`inline-flex items-center gap-2 text-sm ${mc} hover:text-gold-400 transition-colors mb-8`}
                >
                ...
                <div className='flex items-center gap-2 mb-8'>
                    {STEPS.map((s, i) => (
                        <div key={s} className='flex items-center gap-2'>
                            ...
                            {i < STEPS.length - 1 && (
                                <ChevronRight size={14} className={mc} />
                            )}
                        </div>
                    ))}
                </div>
                ...
                <div className='grid lg:grid-cols-3 gap-8'>
                    ...
                    <div className='lg:col-span-2 space-y-6'>
                        ...
                        {step === "address" && (
                            <motion.div
                                ...
                                className={`card-surface rounded-2xl border ${cardBorder} p-6`}
                            >
                                <h2
                                    className={`text-lg font-semibold ${tc} mb-4 flex items-center gap-2`}
                                >
                                    <MapPin
                                        size={16}
                                        ...
                                        {savedAddresses.map((addr) => (
                                            <button
                                                ...
                                                className={`w-full text-left p-3 rounded-xl border transition-all ${
                                                    ...
                                                    :   `${cardBorder} ${innerBg} hover:border-white/20`
                                                }`}
                                            >
                                                <div className='flex items-center justify-between'>
                                                    <div>
                                                        <div
                                                            className={`text-sm font-medium ${tc}`}
                                                        >
                                                            {addr.fullName}
                                                        </div>
                                                        <div
                                                            className={`text-xs ${mc}`}
                                                        >
                                                            {addr.line1},{" "}
                                                            {addr.city},{" "}
                                                            {addr.state} -{" "}
                                ...
                                <div className='grid sm:grid-cols-2 gap-4'>
                                    <div>
                                        <label
                                            className={`block text-xs ${sc} mb-1.5`}
                                        >
                                            Full Name *
                                        </label>
                                        <input
                            ...
                            >
                                ...
                                <div className='space-y-3'>
                                    {PAYMENT_METHODS.map((m) => (
                                        ...
                                        >
                                            <div
                                                className={`w-9 h-9 rounded-lg flex items-center justify-center ${paymentMethod === m.id ? "bg-gold-500/15" : innerBg}`}
                                            >
                                                <m.icon
                                                    size={16}
                                                    className={
                                            ...
                                            <div className='flex-1'>
                                                <div
                                                    className={`text-sm font-medium ${paymentMethod === m.id ? tc : sc}`}
                                                >
                                                    {m.label}
                                                </div>
                                                <div
                                            ...
                                            <div
                                                className={`w-4 h-4 rounded-full border-2 ${paymentMethod === m.id ? "border-gold-500 bg-gold-500" : "border-white/20"}`}
                                            >
                                                {paymentMethod === m.id && (
                                                    <Check
                                                        size={10}
                                                        className='text-dark-700 m-auto'
                                                    />
                                                )}
                                            </div>
                                        </button>
                                ...
                                <div className='flex gap-3 mt-5'>
                                    <button
                                        ...
                                        className='btn-outline-gold px-5 py-3 rounded-xl font-semibold text-sm'
                                    >
                                        Back
                                    </button>
                                    <button
                        ...
                        {step === "review" && (
                            ...
                            >
                                ...
                                >
                                    <h2
                                        className={`text-lg font-semibold ${tc} mb-4`}
                                    >
                                        Review Your Order
                                    </h2>
                                    ...
                                    {/* Address Summary */}
                                    <div
                                        className={`rounded-xl p-4 mb-4 ${innerBg}`}
                                    >
                                        <div
                                            className={`text-xs ${mc} mb-1 flex items-center gap-1`}
                                        >
                                    ...
                                    <div className='space-y-2'>
                                        {items.map((item) => (
                                            <div
                                                ...
                                                className={`flex items-center gap-3 p-2 rounded-lg ${innerBg}`}
                                            >
                                                <div
                                                    className={`w-10 h-10 rounded-lg overflow-hidden flex-shrink-0 ${isDark ? "bg-dark-300" : "bg-gray-200"}`}
                                                >
                                                    {(
                                                        item.product
                                                            .images as string[]
                                                ...
                                                <div className='flex-1 min-w-0'>
                                                    ...
                                                    {item.selectedColor && (
                                                        <div
                                                            className={`text-[10px] ${mc} flex items-center gap-1 ${item.caseBrand ? "" : "mt-0.5"}`}
                                                        >
                                                            <Palette size={9} />{" "}
                                                            {item.selectedColor}
                                                        </div>
                    ...
                    <div className='lg:col-span-1'>
                        <div
                            className={`card-surface rounded-2xl border ${cardBorder} p-5 sticky top-24`}
                        >
                            <h3 className={`text-sm font-semibold ${tc} mb-4`}>
                                Order Summary
                            </h3>
                                ...
                                {items.map((item) => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`rounded-xl p-2.5 ${innerBg}`}
                                        >
                                            <div className='flex items-start gap-2'>
                                                <div
                                                    className={`w-9 h-9 rounded-lg overflow-hidden flex-shrink-0 ${isDark ? "bg-dark-300" : "bg-gray-200"}`}
                                                >
                                                    {(
                                                        item.product
                                                            .images as string[]
                                                ...
                                                <div className='flex-1 min-w-0'>
                                                    <div
                                                        className={`${tc} truncate text-xs font-medium`}
                                                    >
                                                        {item.product.name}
                                                    </div>
                                                    {item.caseBrand &&
                                                    ...
                                                    <div className='flex items-center justify-between mt-1.5'>
                                                        <div className='flex items-center gap-1'>
                                                            ...
                                                            <span
                                                                className={`w-6 text-center text-xs font-semibold ${tc}`}
                                                            >
                                                                {item.quantity}
                                                            </span>
                                                            <button
                            ...
                            {loyaltyPoints >= REDEMPTION_RATE && (
                                <div
                                    className={`rounded-xl p-3 mb-4 border border-gold-500/20 ${isDark ? "bg-gold-500/5" : "bg-gold-50"}`}
                                >
                                    <div className='flex items-center justify-between mb-2'>
                                        <div className='flex items-center gap-1.5'>
                                            ...
                                            />
                                            <span
                                                className={`text-xs font-semibold ${tc}`}
                                            >
                                                Loyalty Points
                                            </span>
                                        </div>
                                    ...
                                    {!useLoyalty ?
                                        ...
                                        >
                                            <Sparkles size={11} /> Use points to
                                            save ₹
                                            {Math.floor(
                                                maxRedeemablePoints /
                                                    REDEMPTION_RATE,
                                    ...
                                    :   <div className='space-y-2'>
                                            <div className='flex items-center gap-2'>
                                                ...
                                                />
                                                <span className='text-xs text-gold-400 font-bold whitespace-nowrap'>
                                                    {pointsToRedeem} pts
                                                </span>
                                            </div>
                                            <div className='flex items-center justify-between'>
                                            ...
                                            <button
                                                ...
                                                className={`text-[10px] ${mc} hover:text-red-400 transition-colors`}
                                            >
                                                Don&apos;t use points
                                            </button>
                                        </div>
                            ...
                            <div
                                className={`space-y-2 pt-4 border-t ${cardBorder}`}
                            >
                                ...
                                <div
                                    className={`flex justify-between pt-2 border-t ${cardBorder}`}
                                >
                                    <span
                                        className={`text-sm font-semibold ${tc}`}
                                    >
                                        Total
                                    </span>
                            ...
                            {shippingFee > 0 && (
                                <div className='mt-3 text-xs text-gold-500 bg-gold-500/10 rounded-lg p-2 text-center'>
                                    Add ₹
                                    {(3000 - total).toLocaleString("en-IN")}{" "}
