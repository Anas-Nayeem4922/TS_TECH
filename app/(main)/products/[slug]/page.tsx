...
export default function ProductDetailPage() {
    ...
    const handleAddToCart = async () => {
        ...
        if (isPhoneCase && !selectedCaseModel) {
        ...
        const chosenColor =
            colorVariants.length > 0 ?
                colorVariants[activeColor]?.color
            :   undefined;
        for (let i = 0; i < quantity; i++)
            await addToCart(product, 1, {
                caseBrand: selectedCaseBrand,
                caseModel: selectedCaseModel,
                selectedColor: chosenColor,
            });
        setAdding(false);
        toast.success("Added to cart!", {
            description: `${quantity} × ${product.name}${selectedCaseModel ? ` (${selectedCaseBrand} ${selectedCaseModel})` : ""}${chosenColor ? ` — ${chosenColor}` : ""}`,
        });
    };
    ...
    const handleDeleteReview = async () => {
        ...
        const res = await fetch(`/api/reviews?id=${userReview.id}`, {
            method: "DELETE",
            cache: "no-store",
        });
        if (!res.ok) {
    ...
    const availableModels =
        selectedCaseBrand ? (PHONE_CASE_BRANDS[selectedCaseBrand] ?? []) : [];
    const generalImages = (product?.images ?? []) as string[];
    const currentImages =
        colorVariants.length > 0 && colorVariants[activeColor] ?
        ...
        <div className='min-h-screen pt-28 pb-16'>
            <div className='max-w-7xl mx-auto px-4 sm:px-6'>
                ...
                <nav className='flex items-center gap-2 py-6 text-sm text-silver-500'>
                    ...
                    <ChevronRight size={14} />
                    <span className='text-silver-400 truncate max-w-40'>
                        {product.name}
                    </span>
                </nav>
                ...
                <div className='grid lg:grid-cols-2 gap-10 mb-16'>
                    ...
                    <div>
                        ...
                        <div className='flex items-baseline gap-3 mb-6'>
                            ...
                            {product.comparePrice && (
                                <span className='text-lg text-silver-600 line-through'>
                                    ₹
                                ...
                                <div className='flex gap-2 flex-wrap'>
                                    {colorVariants.map((variant, i) => (
                                        ...
                                        >
                                            ...
                                            <span
                                                className={`text-xs font-medium ${i === activeColor ? "text-white" : "text-silver-400"}`}
                                            >
                                                {variant.color}
                                            </span>
                                        </button>
                                    ...
                                    <div>
                                        ...
                                        <div className='relative'>
                                            <select
                                                ...
                                                onChange={(e) => {
                                                    setSelectedCaseBrand(
                                                        e.target.value,
                                                    );
                                                    setSelectedCaseModel("");
                                                }}
                                            ...
                                            <ChevronDown
                                                size={16}
                                                className='absolute right-3 top-1/2 -translate-y-1/2 text-silver-500 pointer-events-none'
                                            />
                                        </div>
                                    </div>
                                    <div>
                            ...
                            <div className='mb-6'>
                                ...
                                <div className='flex flex-wrap gap-2'>
                                    {inTheBox.map((item, i) => (
                                        <div
                                            key={i}
                                            className='flex items-center gap-1.5 bg-white/3 border border-white/5 rounded-lg px-3 py-1.5'
                                        >
                                            <Check
                                                size={11}
                                ...
                                <div className='flex items-center gap-3'>
                                    <div className='flex items-center gap-2 bg-dark-400 border border-white/10 rounded-xl p-1'>
                                        <button
                                            ...
                                            className='w-8 h-8 flex items-center justify-center text-silver-400 hover:text-white transition-colors'
                                        >
                                            <Minus size={14} />
                                        </button>
                        ...
                        <div className='grid grid-cols-3 gap-3 mb-6'>
                            {[
                                ...
                                >
                                    ...
                                    />
                                    <span className='text-xs text-silver-500 text-center'>
                                        {b.label}
                                    </span>
                                </div>
                            ))}
                ...
                <div className='mb-16'>
                            ...
                            <div className='flex-1 w-full'>
                                {[5, 4, 3, 2, 1].map((star) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <span className='text-xs text-silver-500 w-8'>
                                                {star}★
                                            </span>
                                            ...
                                            <span className='text-xs text-silver-600 w-6 text-right'>
                                                {count}
                                            </span>
                                        </div>
                                    );
                        ...
                        <div className='card-surface rounded-2xl border border-white/5 p-6 mb-6'>
                            ...
                            {userReview && !editingReview ?
                                <div>
                                    ...
                                    {userReview.body && (
                                        <p className='text-sm text-silver-400 leading-relaxed mb-3'>
                                            {userReview.body}
                                        </p>
                                    )}
                            ...
                            :   <div className='space-y-4'>
                                    <div>
                                        <label className='block text-xs text-silver-400 mb-2'>
                                            Your Rating
                                        </label>
                                        <StarRating
                                            value={reviewRating}
                                    ...
                                    <div className='flex gap-2'>
                                        {userReview && (
                                            <button
                                                ...
                                                className='btn-outline-gold px-4 py-2.5 rounded-xl text-sm'
                                            >
                                                Cancel
                                            </button>
                                        )}
                    ...
                    {reviews.length > 0 ?
                        <div className='grid sm:grid-cols-2 lg:grid-cols-3 gap-4'>
                            {reviews.map((r) => (
                                <div
                                    key={r.id}
                                    className='card-surface rounded-xl p-4 border border-white/5'
                                >
                    ...
                    :   <p className='text-center text-silver-500 py-8'>
