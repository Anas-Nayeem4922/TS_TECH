...
type OrderWithItems = Order & {
    orderItems: (OrderItem & {
        product: { images: string[]; name: string } | null;
    })[];
};
    ...
    useEffect(() => {
        fetch("/api/wishlist", { cache: "no-store" })
            ...
            .then(({ ids }) => {
                ...
                fetch(`/api/products?ids=${idList.join(",")}`, {
                    cache: "no-store",
                })
                    .then((r) => r.json())
                    .then(({ products }) =>
            ...
            <div className='max-w-5xl mx-auto px-4 sm:px-6 py-10'>
                ...
                <div className='flex items-center justify-between mb-8'>
                    <div className='flex items-center gap-4'>
                        ...
                        <div>
                            <h1 className={`text-xl font-bold ${tc}`}>
                                {userName}
                            </h1>
                ...
                {/* Tabs */}
                <div
                    className={`flex gap-1 ${tabBg} rounded-xl p-1 mb-8 overflow-x-auto`}
                >
                    ...
                    <div>
                        ...
                        : orders.length === 0 ?
                            <div className='text-center py-20'>
                                <ShoppingBag
                                    size={48}
                                    className={`${emptyIcon} mx-auto mb-4`}
                                />
                                <h2
                                    className={`text-lg font-semibold ${tc} mb-2`}
                                >
                                    No orders yet
                                </h2>
                                <p className={`${mc} mb-6`}>
                                ...
                                <Link
                                    href='/products'
                                    className='btn-gold inline-flex items-center gap-2 px-5 py-3 rounded-xl font-semibold'
                                >
                                        ...
                                        >
                                            ...
                                            <div className='flex gap-2 mb-4 flex-wrap'>
                                                {(order.orderItems ?? [])
                                                    ...
                                                    .map((item) => (
                                                        ...
                                                        >
                                                            <div
                                                                className={`w-12 h-12 rounded-lg overflow-hidden border ${isDark ? "bg-dark-300 border-white/5" : "bg-gray-100 border-gray-200"}`}
                                                            >
                                                                {item.product
                                                                    ?.images?.[0] && (
                                                                    <img
                                                ...
                                                {(order.orderItems ?? [])
                                                    .length > 4 && (
                                                    <div
                                                        className={`w-12 h-12 rounded-lg border flex items-center justify-center text-xs ${mc} ${isDark ? "bg-dark-300 border-white/5" : "bg-gray-100 border-gray-200"}`}
                                                    >
                                                        +
                                                        {(
                                                            order.orderItems ??
                                            ...
                                            <div className='flex items-center justify-between'>
                                                <div>
                                                    ...
                                                    <span className='mx-2 text-silver-700'>
                                                        •
                                                    </span>
                                                ...
                                                <div
                                                    className={`text-xs ${mc} flex items-center gap-1`}
                                                >
                                                    {(
                                                        order.paymentMethod ===
                                                        "upi"
                                                            ...
                                                            >
                                                                ...
                                                                {i <
                                                                    flow.length -
                                                                        1 && (
                                                                    <div
                                                                        className={`w-4 h-px ${i < currentIndex ? "bg-gold-500/40" : "bg-white/10"}`}
                                                                    />
                                                                )}
                                                            </div>
                                                        );
                    ...
                    <div>
                        {wishlistProducts.length === 0 ?
                            <div className='text-center py-20'>
                                ...
                                <p className={`${mc} mb-6`}>
                                    Click the heart icon on any product to add
                                    it here.
                                </p>
                                <Link
                                    href='/products'
                ...
                {tab === "profile" && (
                    <div
                        className={`card-surface rounded-2xl border ${cardBorder} p-6 max-w-lg space-y-4`}
                    >
                        <h2 className={`text-lg font-bold ${tc}`}>
                            Account Details
                        </h2>
                        {[
                            ...
                            },
                            {
                                ...
                                value: `${loyaltyPoints.toLocaleString("en-IN")} pts (₹${Math.floor(loyaltyPoints / 100) * 50} value)`,
                            },
                        ].map(({ label, value }) => (
                            <div key={label}>
                                <label className={`block text-xs ${sc} mb-1.5`}>
                                    {label}
                                </label>
                                <div
                                    className={`input-dark px-4 py-3 rounded-xl text-sm ${inputBg}`}
                                >
                                    {value}
                                </div>
                            </div>
