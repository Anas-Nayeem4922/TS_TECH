...
export default function EditProductPage() {
    ...
    useEffect(() => {
        Promise.all([
            fetch(`/api/products/manage?id=${id}`, { cache: "no-store" }).then(
                (r) => r.json(),
            ),
            ...
            if (product) {
                ...
                setForm({
                    ...
                    images:
                        (product.images as string[]).length ?
    ...
    const addBoxItem = () => {
        ...
        setForm((p) =>
            p ? { ...p, inTheBox: [...p.inTheBox, boxItem.trim()] } : p,
        );
        setBoxItem("");
    };
    const removeBoxItem = (idx: number) => {
        setForm((p) =>
            p ? { ...p, inTheBox: p.inTheBox.filter((_, i) => i !== idx) } : p,
        );
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!res.ok) {
            ...
            toast.error(
                "Failed to update: " + (errData.error || "Unknown error"),
            );
        } else {
        ...
        <div className='max-w-3xl mx-auto'>
            <div className='flex items-center gap-3 mb-6'>
                ...
                <div>
                    ...
                    <p className='text-silver-500 text-sm truncate max-w-xs'>
                        {form.name}
                    </p>
                </div>
            </div>
