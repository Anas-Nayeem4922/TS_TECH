...
export default function AdminDashboardPage() {
    ...
    const handleDeleteOrder = async () => {
        ...
        try {
            ...
            setRecentOrders((prev) =>
                prev.filter((o) => o.orderNumber !== deleteTarget.orderNumber),
            );
            setStats((prev) => ({
                ...prev,
            ...
            toast.success("Order deleted", {
                description: `Order ${deleteTarget.orderNumber} removed`,
            });
        } catch (err) {
    ...
    const toggleDailyDeal = async (product: Product) => {
        ...
        const res = await fetch("/api/admin/products", {
            ...
            body: JSON.stringify({
                id: product.id,
                data: { isDailyDeal: newDealStatus },
            }),
            cache: "no-store",
        });
        ...
        if (!res.ok) {
            ...
            setAllProducts((prev) =>
                prev.map((p) =>
                    p.id === product.id ?
                        { ...p, isDailyDeal: newDealStatus }
                    :   p,
                ),
            );
            if (newDealStatus) {
                setDailyDealsProducts((prev) => [
                    ...prev,
        ...
        <div className='space-y-6'>
                ...
                {[
                    ...
                    >
                        <div className='flex items-start justify-between mb-3'>
                            <div
                                className={`w-9 h-9 rounded-xl bg-white/5 flex items-center justify-center ${stat.color}`}
                            >
                                <stat.icon size={16} />
                            </div>
                            {stat.href !== "#" && (
            ...
            <div className='card-surface rounded-2xl border border-white/5 p-5'>
                <div className='flex items-center justify-between gap-3'>
                    ...
                    >
                        ...
                        <span className='text-xs text-silver-500 font-normal flex-shrink-0'>
                            ({dailyDealsProducts.length})
                        </span>
                        <ChevronDown
                            size={14}
                            className={`text-silver-500 transition-transform flex-shrink-0 ${dealsExpanded ? "rotate-180" : ""}`}
                        />
                    </button>
                    <button
                ...
                {dailyDealsProducts.length === 0 && !showDealManager && (
                    <p className='text-silver-500 text-sm text-center py-4'>
                        No daily deals selected. Click &quot;Manage&quot; to add
                        products to the daily deals banner.
                    </p>
                )}
                            ...
                            >
                                <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 pt-4'>
                                    {dailyDealsProducts.map((p) => (
                                        <div
                                            key={p.id}
                                            className='flex items-center gap-2 bg-white/3 border border-white/5 rounded-lg px-3 py-2 min-w-0'
                                        >
                ...
                {showDealManager && (
                    <div className='space-y-2 max-h-80 overflow-y-auto mt-4'>
                        {allProducts.map((p) => (
                            <div
                                key={p.id}
                                className='flex items-center gap-3 p-2 rounded-lg bg-white/3 border border-white/5'
                            >
            ...
            <div className='card-surface rounded-2xl border border-white/5 p-5'>
                ...
                {recentOrders.length === 0 ?
                    <p className='text-silver-500 text-sm text-center py-8'>
                        No orders yet.
                    </p>
            ...
            <div className='grid sm:grid-cols-2 gap-4'>
                <Link
                    href='/admin/products/new'
                    className='card-surface rounded-2xl border border-white/5 p-5 hover:border-gold-500/20 transition-all group'
                >
                    <div className='flex items-center gap-3'>
                        ...
                        <div>
                            ...
                            <div className='text-xs text-silver-500'>
                                List a new product on the site
                            </div>
                        </div>
                        <ArrowRight
                            size={16}
                            className='ml-auto text-silver-600 group-hover:text-gold-400 transition-colors'
                        />
                    </div>
                </Link>
                <Link
                    ...
                    >
                        ...
                        >
                            ...
                            <div className='flex items-center justify-between p-5 border-b border-white/5'>
                                ...
                                <button
                                    onClick={() => setSelectedOrder(null)}
                                    className='p-2 text-silver-500 hover:text-white transition-colors rounded-lg hover:bg-white/5'
                                >
                                    <X size={18} />
                                </button>
                            </div>
                            ...
                            <div className='p-5 space-y-5'>
                                    ...
                                    <div className='space-y-2'>
                                        {selectedOrder.orderItems.map(
                                            (item) => (
                                                <div
                                                    key={item.id}
                                                    className='flex items-start gap-3 p-2.5 rounded-lg bg-white/3'
                                                >
                                    ...
                                    {selectedOrder.loyaltyDiscount > 0 && (
                                        <div className='flex items-center justify-between text-xs'>
                                            ...
                                            <span className='text-green-400'>
                                                −₹
                                                {selectedOrder.loyaltyDiscount.toLocaleString(
                                                    "en-IN",
                                                )}
                                            </span>
                                        </div>
                                ...
                                {selectedOrder.notes && (
                                    <div>
                                        ...
                                        <div className='bg-white/3 rounded-lg p-3 text-sm text-silver-300'>
                                            {selectedOrder.notes}
                                        </div>
                                    </div>
                                )}
                ...
                <AlertDialogContent className='bg-dark-500 border border-white/10 text-white'>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription className='text-silver-400'>
                            ...
                            (₹{deleteTarget?.total.toLocaleString("en-IN") ?? 0}
