...
export function ReferralPanel() {
    ...
    const shareCode = async () => {
        if (!myCode) return;
        const text = `🎁 Use my code ${myCode.code} at TS Technology to get ₹${myCode.referredReward} off your first order! Shop premium tech accessories at tstechcanopy.com`;
        if (navigator.share) {
        ...
        <div className='space-y-5 max-w-2xl'>
            ...
            <div>
                ...
                <p className='text-silver-400 text-sm mt-1'>
                    ...
                    <span className='text-gold-400 font-semibold'>₹50</span> for
                    each successful referral, and your friend gets{" "}
                    <span className='text-gold-400 font-semibold'>₹25</span> off
                    their first order.
                </p>
            </div>
            ...
            {!myCode ?
                <div className='card-surface rounded-2xl border border-white/5 p-6 text-center'>
                    ...
                    <p className='text-silver-400 text-sm mb-5'>
                        Create your personal referral code and start earning
                        rewards.
                    </p>
                    <button
                        onClick={createCode}
            ...
            :   <div className='card-surface rounded-2xl border border-gold-500/15 p-5'>
                    <div className='flex items-center justify-between mb-4'>
                        ...
                        <span
                            className={`text-xs px-2 py-0.5 rounded-full ${myCode.isActive ? "bg-green-500/15 text-green-400 border border-green-500/20" : "bg-red-500/15 text-red-400 border border-red-500/20"}`}
                        >
                    ...
                    <div className='relative bg-dark-400 rounded-xl border border-gold-500/20 p-4 mb-4 text-center'>
                        <div className='text-3xl font-bold tracking-widest gold-text font-mono'>
                            {myCode.code}
                        </div>
                        <div className='text-xs text-silver-500 mt-1'>
                            {myCode.usesCount} times used
                        </div>
                    </div>
                    ...
                    <div className='mt-4 grid grid-cols-2 gap-3 text-center'>
                        ...
                        <div className='bg-white/3 rounded-xl p-3 border border-white/5'>
                            <div className='text-sm font-bold text-green-400'>
                                ₹{myCode.referredReward}
                            </div>
                            <div className='text-[10px] text-silver-500 mt-0.5'>
                                Friend gets off first order
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    {[
                        {
                            ...
                            desc: "Get a unique referral code from your account.",
                        },
                        {
                            step: "2",
                            title: "Share with friends",
                            desc: "Send your code via WhatsApp, Instagram, or any platform.",
                        },
                        {
                            step: "3",
                            title: "Friend signs up",
                            desc: "Your friend creates an account and applies your code.",
                        },
                        {
                            step: "4",
                            title: "Both earn rewards",
                            desc: "Friend gets ₹25 off. You get ₹50 credited to your wallet.",
                        },
                    ].map(({ step, title, desc }) => (
                        ...
                        {uses.map((use, i) => (
                            ...
                            >
                                ...
                                <div className='text-right'>
                                    <div
                                        className={`text-xs font-medium capitalize ${use.status === "credited" ? "text-green-400" : "text-yellow-400"}`}
                                    >
                                        {use.status === "credited" ?
                                            `+₹${use.referrerReward}`
                                        :   "Pending"}
                    ...
                    <div className='space-y-2 max-h-48 overflow-y-auto'>
                        {credits.map((credit) => (
                            ...
                            >
                                ...
                                <div
                                    className={`text-sm font-semibold ${credit.isUsed ? "text-silver-600 line-through" : "text-green-400"}`}
                                >
                                    {credit.isUsed ?
                                        "Used"
                                    :   `+₹${credit.amount}`}
